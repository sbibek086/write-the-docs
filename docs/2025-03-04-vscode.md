...
---
 ...
 grep -rnw . --exclude-dir={node_modules,.git,dist} -e "auth" inside search button beside dirTree in Vscode, to search auth string across all files inside all folders (excluding .git etc) in prj
